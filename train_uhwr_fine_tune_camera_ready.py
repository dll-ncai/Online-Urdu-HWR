    ...
    for batch in tqdm(data_loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            if decode_mode == "greedy":
                ...
                outputs = beam_search_decode(model, pixel_values, tokenizer, device=device)
...
def main():
    ...
    # train_df = pd.read_csv(f"{DATA_ROOT}/data_online/train.csv")
    ...
    # eval_df = pd.read_csv(f"{DATA_ROOT}/data_online/val.csv")
    eval_df = pd.read_csv(f"{DATA_ROOT}/val_leakproof.csv")

    tokenizer = get_tokenizer()

    train_dataset = COHWRDataset(DATA_ROOT, train_df, tokenizer, aug=True, img_feat="img_stroke", aux_feat=[])
    eval_dataset = COHWRDataset(DATA_ROOT, eval_df, tokenizer, img_feat="img_stroke", aux_feat=[])
    ...
    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True,
                              collate_fn=ocollate_fn, num_workers=4, worker_init_fn=seed_worker, generator=g)
    ...
    model.load_state_dict(torch.load("best_model_uhwr_icdar.pt", map_location=device))
    ...
    for epoch in range(max_epochs):
        ...
        val_loss, val_cer = evaluate(
            model, eval_loader, tokenizer, device, loss_fn, decode_mode="beam_search", data="img_stroke"
        )
        ...
        if improved:
            ...
            patience_ctr = 0
            torch.save(model.state_dict(), f"best_model_uhwr_icdar_online_finetune_camera_ready_{SEED}.pt")
    ...
    filename = f"loss_curve_uhwr_icdar_online_finetune_camera_ready_{SEED}_{len(train_loss_history)}epochs.png"
